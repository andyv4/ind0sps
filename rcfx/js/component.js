...
ui.autocomplete_keyup = function(e){
  ...
  if(this.value.length == 0)
    ...
    window.setTimeout("ui.autocomplete_hint(ui('$" + ui.uiid(el) + "'), \"" + this.value + "\")", 300);
}
ui.autocomplete_hint = function(el, hint){
  if(hint.length > 2 && el.firstElementChild.value == hint){
    ...
    if(prehint.length > 0 && eval("typeof " + prehint) == "function"){
      var f = eval(prehint);
      prehint = f(el);
    ...
    //el.setAttribute("data-value", el.firstElementChild.value);
...
ui.autocomplete_menuitemclick = function(e){
  ...
  el.firstElementChild.value = this.innerHTML;
...
ui.datepicker_selector_content = function(d, el, current_d){
  ...
  var d4 = 1;
  var d5 = ui.uiid(el);
  var d8 = new Date(d.getTime()); d8.setMonth(d8.getMonth() - 1);
  var d9 = new Date(d.getTime()); d9.setMonth(d9.getMonth() + 1);
  var c = "";
  ...
  c += "<table class='datetable' cellspacing='1' width='100%'>";
  c += "<thead><tr>";
  ...
  for(var i = 0 ; i < 6 ; i++){
    ...
    for(var j = 0 ; j < 7 ; j++){
      ...
      if(d1 > 0) d1--;
        ...
        var active = formatDate(current_d, 'yyyyMMdd') == d6 ? 1 : 0;

        c += "<label onclick=\"ui.datepicker_ondateselect(ui('$" + d5 + "'), '" + d6 + "');\" class='day" + (active ? ' active': '') + "'>" + d4 + "</label>";
        d4++;
      }
      c += "</td>";
    }
  ...
  c += "</tbody>";
  c += "<tfoot><tr><td colspan='7'><label class='misc' onclick=\"ui.datepicker_ondateselect(ui('$" + d5 + "'), '" + (formatDate(new Date(), 'yyyyMMdd')) + "');\">Today</label></td></tr></tfoot>";
  c += "</table>";
  ui.datepicker_selector.innerHTML = c;
}
ui.datepicker_value = function(el){
  return formatDate(getDateFromFormat(el.firstElementChild.innerHTML, 'd NNN yyyy'), 'yyyyMMdd');
}
...
ui.dropdown_menuitemclick = function(e){
  var popup = this.parentNode;
  var el = popup.parentNode;
  el.firstElementChild.nextElementSibling.innerHTML = this.innerHTML;
  el.setAttribute("data-value", this.getAttribute("data-value"));
  popup.classList.add('off');
...
ui.dropdown_setvalue = function(el, obj){
  if(el.getAttribute("data-items").length > 0){
    ...
    if(item != null){
      ...
      el.firstElementChild.nextElementSibling.innerHTML = item['text'];
    }
  }
  else{
    el.setAttribute("data-value", obj['value']);
    el.firstElementChild.nextElementSibling.innerHTML = obj['text'];
  }
}
...
ui.modal_open = function(el, params){
  ...
  if(params.autoheight && scrollable){
    scrollable.style.height = (window.innerHeight *.75) + "px";
  }
  ...
  el.addEventListener('animationend', function(){ ui.modal_close_ex(el); });
  el.addEventListener('webkitAnimationEnd', function(){ ui.modal_close_ex(el); });
  el.addEventListener('oAnimationEnd', function(){ ui.modal_close_ex(el); });
}
...
ui.popupopen = function(popup, refEl, options){
  ...
  if(popup.clientHeight <= availablespace_bottom){
    changed_properties['top'] = top + refEl.clientHeight + "px";
  }
  else{
    changed_properties['top'] = top - popup.clientHeight;
  }
...
ui.textbox_onblur = function(e, input){
  ...
  var precision = value - Math.floor(value) > 0 ? 2 : 0;
  var datatype = el.getAttribute("data-datatype");
  switch(datatype){
    case 'number':
    case 'money':
...
ui.toggler_setvalue = function(el, value){
  el.firstElementChild.className = value == 1 ? "item right" : "item left";
}
ui.toggler_ontoggle = function(e, el){
  el.firstElementChild.className = el.firstElementChild.classList.contains('left') ? 'item right' : 'item left';
  if(el.hasAttributes("data-onchange")){
    var name = el.hasAttribute("data-name") ? el.getAttribute("data-name") : '';
    ui.eventcall(el.getAttribute("data-onchange"), { name:name, value:el.firstElementChild.classList.contains('left') ? 0 : 1 }, el);
  }
}
